...
public abstract class MidnightConfig {
    ...
    public static void init(String modid, Class<? extends MidnightConfig> config) {
        path = PlatformSpecificHelper.getConfigDirectory().resolve(modid + ".json");
        configClass.put(modid, config);

        for (Field field : config.getFields()) {
            EntryInfo info = new EntryInfo();
            if ((field.isAnnotationPresent(Entry.class) || field.isAnnotationPresent(Comment.class)) && !field.isAnnotationPresent(Server.class) && !field.isAnnotationPresent(Hidden.class) && PlatformSpecificHelper.isClient())
    ...
    public void writeChanges(String modid) {
        try { if (!Files.exists(path = PlatformSpecificHelper.getConfigDirectory().resolve(modid + ".json"))) Files.createFile(path);
    ...
    public static class MidnightConfigScreen extends Screen {
        ...
        @Override
        public void close(){
            // I tend to press ESCAPE to exit menus a lot and I prefer the config being saved when I do that
            write(this.modid);
        ...
        public void init() {
            ...
            this.addDrawableChild(ButtonWidget.builder(ScreenTexts.CANCEL, button -> this.closeWithoutSaving()).dimensions(this.width / 2 - 154, this.height - 26, 150, 20).build());
